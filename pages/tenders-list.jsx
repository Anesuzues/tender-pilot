...
function TendersList({ onNav }) {
  ...
  const tenders = TENDERS.filter(t => filter === "all" || t.status === filter);
  ...
  return (
    <div className="page">
      <PageHeader
        eyebrow="Workspace"
        title="Tenders"
        subtitle={`${TENDERS.length} active opportunities · R 251M total pipeline value`}
        actions={
          <>
        ...
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)} className={cx("btn btn-sm", filter === f ? "btn-dark" : "btn-ghost")}>
            ...
            <span className="mono" style={{ opacity: .6, marginLeft: 4 }}>
              {f === "all" ? TENDERS.length : TENDERS.filter(t => t.status === f).length}
            </span>
          </button>
        ))}
        <div style={{ flex: 1 }}/>
      ...
      {view === "table" ? (
        <div className="card" style={{ padding: 0 }}>
          <div className="scrollx">
            <table className="table">
              ...
              <tbody>
                {tenders.map(t => (
                  <tr key={t.id} onClick={() => onNav("analysis")} style={{ cursor: "pointer" }}>
                    <td>
        ...
        <div className="grid g-2">
          {tenders.map(t => (
            <div key={t.id} className="card card-pad" style={{ cursor: "pointer" }} onClick={() => onNav("analysis")}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  ...
                  <div className="mono tnum" style={{ fontSize: 14, fontWeight: 600, marginTop: 2 }}>{t.closingDays}d</div>
                </div>
