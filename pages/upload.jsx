  ...
  const startUpload = () => {
    ...
    const id = setInterval(() => {
      setProgress(p => {
        if (p >= 100) {
          clearInterval(id);
          setTimeout(() => setStage("processing"), 200);
          return 100;
        }
        return p + 7;
      });
    ...
    <div className="page">
      ...
      {stage === "idle" && (
        <div className="card" style={{
          padding: 0, border: "2px dashed var(--border-strong)", borderRadius: 16, overflow: "hidden",
          background: "linear-gradient(135deg, var(--surface) 60%, color-mix(in oklab, var(--emerald-soft), var(--surface) 70%))",
        }}>
          <div style={{ padding: 60, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.014em" }}>
              Drag &amp; drop your tender file here
            </div>
            <div style={{ fontSize: 13, color: "var(--text-2)", marginTop: 8 }}>
              PDF up to 50MB · also DOCX, ZIP, or scanned images · OCR + AI extract in &lt;2 minutes
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 22 }}>
              <button className="btn btn-primary" onClick={startUpload}><Icon.upload size={13}/> Choose file</button>
              <button className="btn"><Icon.external size={13}/> Paste eTenders URL</button>
            </div>
      ...
      {stage === "uploading" && (
        <div className="card card-pad" style={{ padding: 40 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 22 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600 }}>SARS_RFB_2025_IT_0142.pdf</div>
              <div className="mono" style={{ fontSize: 11, color: "var(--text-3)" }}>12.4 MB · 187 pages</div>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 16 }} className="up-grid">
          <div className="card card-pad" style={{ padding: 30 }}>
              ...
              {steps.map((s, i) => (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{
                    ...
                    background: s.status === "done" ? "var(--emerald-soft)" : s.status === "active" ? "var(--blue-soft)" : "var(--surface-2)",
                    color: s.status === "done" ? "var(--emerald)" : s.status === "active" ? "var(--blue)" : "var(--text-3)",
                  ...
                  <div style={{ fontSize: 13, color: s.status === "pending" ? "var(--text-3)" : "var(--text)", fontWeight: s.status === "active" ? 600 : 500 }}>
                    {s.t}
                  </div>
            ...
            {stage === "done" && (
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button className="btn btn-primary" onClick={() => onNav("analysis")}>
          ...
          <div className="card card-pad">
            ...
            <div style={{
              ...
              background: "var(--surface-2)", padding: 16, overflow: "hidden",
              position: "relative",
            }}>
              <div className="mono" style={{ fontSize: 9, color: "var(--text-3)", marginBottom: 8 }}>REQUEST FOR BID · RFB 2025/IT/0142</div>
              <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: "var(--text)" }}>Supply, Delivery and Maintenance of Cybersecurity Infrastructure</div>
              <div style={{ fontSize: 8.5, color: "var(--text-2)", lineHeight: 1.5 }}>
                {Array.from({length:18}).map((_, i) => (
                  <div key={i} style={{ height: 4, background: "var(--border)", borderRadius: 2, width: `${65 + Math.random()*30}%`, marginBottom: 3, opacity: 0.5 + Math.random()*0.4 }}/>
                ))}
              </div>
              <div style={{ position: "absolute", bottom: 12, right: 12 }} className="mono" style={{ fontSize: 9, color: "var(--text-3)" }}>p. 1 / 187</div>
              {stage === "processing" && (
      ...
      <div className="card" style={{ marginTop: 16, padding: 0 }}>
        <div className="between" style={{ padding: "14px 18px" }}>
          ...
          <button className="btn btn-sm btn-ghost">View all<Icon.arrow size={12}/></button>
        </div>
        <table className="table">
          <thead>
            <tr><th>File</th><th>Tender</th><th>Uploaded</th><th>Status</th><th></th></tr>
          </thead>
          <tbody>
            {[
              { f: "Eskom_SCADA_RFQ_0337.pdf", t: "Maintenance of SCADA Systems", time: "23 min ago", status: "in-review" },
              { f: "JHB_Security_2026.pdf", t: "City of Joburg · Physical Security", time: "yesterday", status: "draft" },
              { f: "Mthatha_Phase2.zip", t: "DPWI Mthatha Office Phase 2", time: "May 20", status: "flagged" },
              { f: "Tygerberg_Cleaning.pdf", t: "Tygerberg Hospital Cleaning Services", time: "May 18", status: "shortlisted" },
            ].map((r, i) => (
              <tr key={i}>
                <td><div className="mono" style={{ fontSize: 12 }}>{r.f}</div></td>
                <td style={{ fontSize: 12.5 }}>{r.t}</td>
                <td className="mono" style={{ fontSize: 11.5, color: "var(--text-3)" }}>{r.time}</td>
