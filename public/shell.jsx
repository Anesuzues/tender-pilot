...
function Sidebar({ activePage, onNav, collapsed }) {
  return (
    <aside className="sidebar">
      <div style={{ padding: "16px 14px 12px", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {!collapsed && (
          <div>
            ...
            <div style={{ fontSize: 10.5, color: "var(--text-3)", marginTop: 3, letterSpacing: ".05em" }}>AI · v2.4</div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: 10, borderTop: "1px solid var(--border)" }}>
        <div className="ai-glow" style={{ padding: 12, borderRadius: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 600 }}>AI Credits</div>
          </div>
          <div style={{ fontSize: 11.5, color: "var(--text-2)", marginBottom: 8 }}>
            <span className="mono tnum">3,820</span> of <span className="mono tnum">5,000</span> used
          </div>
          <Bar value={76} color="var(--emerald)"/>
      ...
      <div style={{ padding: "8px 10px 12px", borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12.5, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Lerato Mokoena</div>
          <div style={{ fontSize: 10.5, color: "var(--text-3)" }}>Sandile Cybersecurity (Pty) Ltd</div>
        </div>
        <button className="btn btn-ghost btn-icon"><Icon.chevDown size={14}/></button>
      </div>
    </aside>
  );
}

function Topbar({ pageTitle, breadcrumb, onCmd, onTheme, theme, onMenu }) {
  return (
    <div className="topbar">
      <button className="btn btn-ghost btn-icon" onClick={onMenu} style={{ display: "none" }}><Icon.panelLeft size={16}/></button>
      ...
      <div style={{ flex: 1 }}/>
      <button className="btn btn-ghost btn-sm" onClick={onCmd}>
        <Icon.search size={13}/>
        Quick find
      ...
      <button className="btn btn-ghost btn-icon" title="Notifications" style={{ position: "relative" }}>
        <Icon.bell size={15}/>
        <span style={{ position: "absolute", top: 6, right: 6, width: 6, height: 6, background: "var(--red)", borderRadius: 999, border: "1.5px solid var(--bg-elev)" }}/>
      </button>
...
function CommandPalette({ open, onClose, onNav }) {
  ...
  const allItems = [
    ...
    ...TENDERS.map(t => ({ id: t.id, label: t.title, kind: "Tender", to: "analysis" })),
    ...
    <div className="cmd-backdrop" onClick={onClose}>
      <div className="cmd" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ maxHeight: "50vh", overflowY: "auto", padding: 6 }}>
          ...
          {filtered.map((it, i) => (
            <div key={i}
                 onClick={() => { if (it.to) onNav(it.to); onClose(); }}
                 style={{
